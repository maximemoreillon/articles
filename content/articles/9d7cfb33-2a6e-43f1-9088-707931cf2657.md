...
In this article, MinIO will be used as an example. MinIO pods do not support the `kubectl cp` command as the container does not have `tar` installed, which `kubectl cp` requires to stream files. For applications that do support the `kubectl cp` command, the procedure in this article can be simplified
