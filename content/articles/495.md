...
Let’s imagine a simple Express application which serves `Sensitive data` upon receiving an HTTP GET requests on the _/data_ endpoint:
...
<script>
  export default {
    ...
    methods: {
      get_data() {
        this.axios
          .get("http://YOUR_SERVER_IP:7070/data")
          .then(({ data }) => {
            this.message = data;
          })
          .catch((error) => {
...
All three are packaged as docker containers and can thus be easily deployed using the following _docker-compose.yml_ file:
...
Note the JWT_SECRET environment variable that users will want to set to a secret of their choice. Also, the MONGODB_URL environment variable can be changed so as to use another MongoDB instance.
...
Although modifiable via environment variables, the default username and password are both _admin._
...
More often than not, content is to be serves to multiple users. Accounts for those users can be created using the corresponding button. In this example, we’ll create the user _roger._
...
We will start with the back-end integration. Here the objective is to prevent unauthenticated usersfrom accessing retriving data from the _/data_ endpoint. For Express applications, [a dedicated module](https://github.com/maximemoreillon/express_identification_middleware) has been developed and can be installed via NPM:
...
With this done, the front-end should no longer be able to display the `Sensitive data` from the _/data_ endpoint and return a 403 (forbidden) error code instead:
...
```html
<template>
  <AuthenticationWall :options="wall_options" @user="get_data()">
    <div>
      <h1>My app</h1>
      <p>{{message}}</p>
    </div>
...
Here, one can log in using any user account listed in the user management service, such as the _roger_ user previously created. Upon successful login, the back-end data is finally visible to the user.
