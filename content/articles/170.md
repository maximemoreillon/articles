...
```
sudo mysql -uroot -p$mariadbrootpassword -e "UPDATE mysql.user SET Password = PASSWORD('$mariadbrootpassword') WHERE User = 'root'"
sudo mysql -uroot -p$mariadbrootpassword -e "DELETE FROM mysql.user WHERE User=''"
sudo mysql -uroot -p$mariadbrootpassword -e "DELETE FROM mysql.user WHERE User='root' AND Host NOT IN ('localhost', '127.0.0.1', '::1')"
sudo mysql -uroot -p$mariadbrootpassword -e "DROP DATABASE IF EXISTS test"
sudo mysql -uroot -p$mariadbrootpassword -e "FLUSH PRIVILEGES"
```
